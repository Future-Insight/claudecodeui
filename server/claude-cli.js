...
// Generate Claude environment variables from configuration files
async function generateClaudeEnvVars() {
  try {
...
async function spawnClaude(command, options = {}, ws) {
  return new Promise((resolve, reject) => {
    ...
    // Generate and merge environment variables from configuration
    const claudeEnvVars = await generateClaudeEnvVars();
    const processEnv = {
      ...process.env, // Inherit all existing environment variables
      ...claudeEnvVars // Override with Claude-specific configuration
    };
    
    if (Object.keys(claudeEnvVars).length > 0) {
      console.log('🌍 Using Claude environment variables:', Object.keys(claudeEnvVars));
    }
